...
function generate_daily_predictions(result, start_idx=1, count=864_000)
    ...
    # 计算起始时间（只从活跃传感器获取）
    active_times = [result.data.times[i] for i in result.active_indices if !isempty(result.data.times[i])]
    if isempty(active_times)
        error("No active sensor data available!")
    end
    t_start = minimum(minimum.(active_times))
    ...
    println("  ✓ 全局传感器数: $(result.data.S)")
    println("  ✓ 活跃传感器数: $(length(result.active_indices))")
    println("  ✓ 零负荷传感器数: $(result.data.S - length(result.active_indices))")

    @showprogress for s in 1:result.data.S
        name = result.data.sensor_names[s]
        sensor_type = String(result.data.sensor_types[s])
        is_zero = result.data.is_zero_load[s]
        
        if is_zero
            # ✅ 零负荷传感器：返回零值预测
            daily_predictions["sensors"][name] = Dict(
                "type" => sensor_type,
                "prediction_mean" => zeros(Float32, length(t_grid)),
                "prediction_std" => zeros(Float32, length(t_grid)),
                "is_zero_load" => true,
                "ci_lower" => zeros(Float32, length(t_grid)),
                "ci_upper" => zeros(Float32, length(t_grid))
            )
        else
            # ✅ 活跃传感器：必须使用映射后的活跃索引
            if !haskey(global_to_active, s)
                @warn "传感器 $name (全局索引 $s) 不在 active_indices 中，但 is_zero_load=false！"
                daily_predictions["sensors"][name] = Dict(
                    "type" => sensor_type,
                    "prediction_mean" => zeros(Float32, length(t_grid)),
                    "prediction_std" => zeros(Float32, length(t_grid)),
            ...
            try
                # ������ 关键：使用 active_idx（1-125）而不是 s（1-186）
                μ_pred, σ_pred = multitask_gp_predict(result, active_idx, t_grid)
                
                # 验证维度
                if length(μ_pred) != length(t_grid)
                    @warn "传感器 $name 维度不匹配: μ_pred=$(length(μ_pred)), t_grid=$(length(t_grid))"
                    if length(μ_pred) > length(t_grid)
                        μ_pred = μ_pred[1:length(t_grid)]
                        σ_pred = σ_pred[1:length(t_grid)]
                    else
                        μ_pred = vcat(μ_pred, fill(NaN32, length(t_grid) - length(μ_pred)))
                        σ_pred = vcat(σ_pred, fill(NaN32, length(t_grid) - length(σ_pred)))
                    end
                end
                ...
                @warn "传感器 $name 预测失败 (全局索引=$s, 活跃索引=$active_idx): $e"
                # 打印堆栈（只显示前 3 行）
