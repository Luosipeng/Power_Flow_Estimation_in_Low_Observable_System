...
function evaluate_noise_and_missing_data_impact_corrected(data::MultiSensorData; 
    ...
    for (i, s) in enumerate(eval_sensor_indices[1:min(5, end)])
    ...
    for (i, noise_level) in enumerate(noise_levels)
        ...
        for (j, missing_pct) in enumerate(missing_percentages)
            println("\n[Noise $(Int(noise_level*100))% | Missing $(Int(missing_pct*100))%] Running $n_trials trials...")
            
            # 用于存储本轮所有试验的平均RMSE（针对所有目标传感器的平均）
            trial_mtgp_rmses = Float64[]
            trial_linear_rmses = Float64[]
            
            # 用于存储本轮所有试验的传感器级别详细数据
            # 结构：[trial_idx][sensor_idx] -> rmse
            ...
            for t in 1:n_trials
                ...
                for s in eval_sensor_indices
                    if !isempty(removed_times[s])
                        ...
                        if !isempty(original_clean_values)
                            ...
                            μ_mtgp, _ = icm_predict(mtgp_result, s, query_times)
                            mtgp_rmse = sqrt(mean((μ_mtgp .- original_clean_values).^2))
                            
                            μ_linear, _ = linear_predict(linear_result, s, query_times)
                ...
                if !isempty(current_trial_mtgp_sensor_rmses)
                    push!(trial_mtgp_rmses, mean(current_trial_mtgp_sensor_rmses))
                    push!(trial_linear_rmses, mean(current_trial_linear_sensor_rmses))
                    
                    push!(trial_mtgp_sensor_details, current_trial_mtgp_sensor_rmses)
                    push!(trial_linear_sensor_details, current_trial_linear_sensor_rmses)
                end
            ...
            if !isempty(trial_mtgp_rmses)
                ...
                # Store the first trial's breakdown or average? 
                # Keeping structure consistent: store list of lists (one per trial)
                results["mtgp_rmse_by_sensor"][i, j] = trial_mtgp_sensor_details
                results["linear_rmse_by_sensor"][i, j] = trial_linear_sensor_details
                
                println("   Results (Avg of $n_trials trials):")
                println("    易 MTGP RMSE: $(round(avg_mtgp * 100, digits=4))% (±$(round(std_mtgp*100, digits=4)))")
...
ds = extract_requested_dataset_opendss(
    voltage_mag_a, voltage_mag_b, voltage_mag_c,
            voltage_ang_a, voltage_ang_b, voltage_ang_c,
            power_p_a, power_p_b, power_p_c,
            power_q_a, power_q_b, power_q_c; pmu_buses, scada_buses, ami_buses, pmu_phases, scada_phases, ami_phases
)
