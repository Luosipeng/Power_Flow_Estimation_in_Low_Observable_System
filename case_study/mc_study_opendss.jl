...
pmu_buses, scada_buses, ami_buses, pmu_phases, scada_phases, ami_phases, pmu_sensors, scada_sensors, ami_sensors = FAD20_config_ieee123()
println("\n[1] 📂 Loading data...")
...
noise_levels = [0.01, 0.05]  # 1% and 10% of signal standard deviation
missing_percentages = [0.6]
mtgp_epochs = 100
# tolerance = 1e-6
c = 1e-7
d = 1e-7
FAD = 0.2  # Base MVA for FAD20 system

function add_gaussian_noise(data::MultiSensorData, noise_level::Float64; seed::Int=42)
    Random.seed!(seed)
    
    # ✅ 修正：包含 is_zero_load 字段
...
for (i, noise_level) in enumerate(noise_levels)
    ...
    for (j, missing_pct) in enumerate(missing_percentages)
        ...
        GP_miae_power, GP_miae_theta, GP_mape_voltage, picp_theta, picp_voltage, mpiw_theta, mpiw_voltage = mtgpbmc(mtgp_result, noise_level,lack_a, lack_b, lack_c)
        ...
        println("Noise Level = $(Int(noise_level*100))% | GP PICP Theta    = $(picp_theta)%")
        println("Noise Level = $(Int(noise_level*100))% | GP PICP Voltage  = $(picp_voltage)%")
        println("Noise Level = $(Int(noise_level*100))% | GP MPIW Theta    = $(mpiw_theta)%")
        println("Noise Level = $(Int(noise_level*100))% | GP MPIW Voltage  = $(mpiw_voltage)%")
        println("-"^20)
